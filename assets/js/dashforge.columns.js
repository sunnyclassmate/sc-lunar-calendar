$(function(){
  ...
  if(window.matchMedia('(min-width: 992px)').matches) {
    $('.columns-list .media:first-of-type').addClass('active');
  }


  const contactSidebar = new PerfectScrollbar('.columns-sidebar-body', {
    suppressScrollX: true
  });

  new PerfectScrollbar('.columns-content-body', {
    suppressScrollX: true
  });

  new PerfectScrollbar('.columns-content-sidebar', {
    suppressScrollX: true
  });

  // $('.columns-navleft .nav-link').on('shown.bs.tab', function(e) {
  //   contactSidebar.update()
  // })

  // UI INTERACTION
  // $('.columns-list .media').on('click', function(e) {
  //   e.preventDefault();
  //
  //   $('.columns-list .media').removeClass('active');
